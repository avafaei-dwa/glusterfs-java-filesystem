...
public class Example {
	...
	public static void main(String[] args) throws URISyntaxException,
		...
		properties.load(Example.class.getClassLoader().getResourceAsStream(
				"example.properties"));
		...
		String query = "?uid=0&gid=0";

		String mountUri = "gluster://" + vagrantBox + ":" + volname + "/";
		String testUri = "gluster://" + vagrantBox + ":" + volname + "/baz";
		Path mountPath = Paths.get(new URI(mountUri+query));
        Map<String, String> fileMap = new HashMap<String, String>();
        fileMap.put("uid", "0");
        fileMap.put("gid", "0");
        
		
		FileSystem fileSystem = FileSystems.newFileSystem(new URI(mountUri+query),
				fileMap);
		...
		boolean isHidden = fileSystem.provider().isHidden(
				new GlusterPath(((GlusterFileSystem) fileSystem), hidden));
		System.out.println("Is " + hidden + " hidden? " + isHidden);

		hidden = "/foo/bar";
		isHidden = fileSystem.provider().isHidden(
				new GlusterPath(((GlusterFileSystem) fileSystem), hidden));
		System.out.println("Is " + hidden + " hidden? " + isHidden);
		...
		Path glusterPath = Paths.get(new URI(testUri+query));
		...
		try {
			...
			Files.createFile(glusterPath);
			System.out.println("File created");
		} catch (IOException e) {
			System.out.println("File exists, created at "
					+ Files.getLastModifiedTime(glusterPath));
		}
		String hello = "Hello, ";
		Files.write(glusterPath, hello.getBytes(), StandardOpenOption.WRITE,
				StandardOpenOption.TRUNCATE_EXISTING);
		String world = "world!";
		Files.write(glusterPath, world.getBytes(), StandardOpenOption.WRITE,
		...
		System.out.println("Last modified: "
				+ Files.getLastModifiedTime(glusterPath) + " (should be now)");
		fileSystem.provider().checkAccess(glusterPath, AccessMode.READ,
				AccessMode.WRITE);
		...
		Path symlinkPath = Paths.get(new URI(mountUri + "symlink3"+query));
		Path symlinkTarget = Paths.get(new URI(mountUri + "symlinktarget3"+query));
		Files.createSymbolicLink(symlinkPath, symlinkTarget);
		System.out.println("SYMLINK: " + symlinkPath.toString() + " => "
				+ Files.readSymbolicLink(symlinkPath));

		Path copyPath = glusterPath.resolveSibling("copy");
		System.out.println("!!!!!----- COPY PATH -----: "+copyPath.toUri());  
		...
		System.out.println("Source and copy are "
				+ (bazSize == copySize ? "" : "NOT") + " equal.");
		...
		Path one = Paths.get(new URI("gluster://" + vagrantBox + ":" + volname
				+ "/one" + query));
        Files.createDirectory(one);
		System.out.println("STARTSWITH empty: " + one.startsWith("/"));
		one.register(watchService, StandardWatchEventKinds.ENTRY_CREATE,
